...
public interface IProjectEmployeeRepository extends JpaRepository<ProjectEmployee, Long>  {
    ...
    @Query(nativeQuery = true, value = "SELECT DISTINCT pe.* FROM project_employee pe WHERE pe.project_id = :project_id AND :role IN (SELECT roles_name FROM users_roles WHERE users_id = pe.employee_id)")
